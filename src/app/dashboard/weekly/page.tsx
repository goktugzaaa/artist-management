...
export default async function WeeklyPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-semibold text-neutral-900">Haftalik Plan</h1>
        <p className="mt-1 text-sm text-neutral-500">{plans.length} kayit</p>
      </div>

      <form action={createWeeklyPlan} className="grid gap-3 rounded-2xl border border-neutral-200 bg-white p-5 sm:grid-cols-2">
      ...
      <div className="overflow-hidden rounded-2xl border border-neutral-200 bg-white">
        <table className="w-full text-sm">
          ...
          <tbody>
            {plans.length === 0 && (
              <tr><td colSpan={8} className="px-4 py-8 text-center text-neutral-400">Henuz plan yok.</td></tr>
            )}
            {plans.map((p) => {
              const diff = Number(p.actual_hours) - Number(p.planned_hours);
              return (
                <tr key={p.id} className="border-t border-neutral-100">
                  <td className="px-4 py-3 text-neutral-600">{p.week_start}</td>
                  <td className="px-4 py-3 font-medium text-neutral-900">{p.artists?.name ?? "-"}</td>
                  <td className="px-4 py-3 text-neutral-600">{p.focus ?? "-"}</td>
                  <td className="px-4 py-3 text-neutral-600">{p.planned_hours}</td>
                  <td className="px-4 py-3 text-neutral-600">{p.actual_hours}</td>
                  <td className={`px-4 py-3 font-medium ${diff < 0 ? "text-red-500" : "text-green-600"}`}>
                    {diff > 0 ? `+${diff}` : diff}
                  </td>
                  <td className="px-4 py-3 text-neutral-600">{planStatusLabel[p.status]}</td>
